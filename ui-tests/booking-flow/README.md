...
- **BDD** (`features/*.feature` + `tests/bdd/`): los escenarios "de portada" (reserva exitosa, creación de habitación) también en Gherkin con `pytest-bdd`, legible por alguien de negocio sin tocar Python — mismo flujo que las suites de arriba, no reemplaza los casos negativos/baterías, que se quedan en pytest plano. Corre tanto local como en CI.
- **Negativos** (`tests/booking/test_booking_negative.py`, `tests/admin/test_admin_negative.py`): teléfono con longitud inválida (10 dígitos, rechazado) y login de administrador con credenciales incorrectas.
...
**Validación de teléfono.** El campo `Phone` del formulario de contacto rechaza números de 10 dígitos y acepta 11. Verificado contra la app real: al enviar un teléfono de 10 dígitos, la petición `POST /api/booking` responde `400` y la SPA muestra el mensaje `"size must be between 11 and 21"` junto al campo (el rango real es 11-21 caracteres, no exactamente "10 vs 11"). Cubierto explícitamente en `tests/booking/test_booking_negative.py`.

**Login de administrador con credenciales inválidas no muestra ningún mensaje de error en el DOM.** La respuesta de `POST /api/auth/login` sí trae el motivo (`{"error":"Invalid credentials"}`), pero la SPA nunca lo renderiza — no hay ningún texto de error que assertar. La señal real de que el login falló es negativa: el formulario de login sigue visible y la navegación de administrador (`Rooms`/`Report`) nunca aparece. `tests/admin/test_admin_negative.py` verifica exactamente eso.
...
✅ Booking flow, Admin, batería de datos parametrizada, Docker, BDD (local y en CI) y casos negativos explícitos (teléfono inválido, credenciales de admin incorrectas) funcionando. Ver [plan de estudio](../../README.md).
